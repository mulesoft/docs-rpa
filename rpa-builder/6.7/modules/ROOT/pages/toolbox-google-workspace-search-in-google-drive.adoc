...
+
The *Insert filters* and *Insert operators* dropdown lists contain the most common filters and operators. However, you can use any supported Google https://developers.google.com/workspace/drive/api/guides/ref-search-terms[search query terms and operators].
...
+
Use the wizard for assistance while building your query. You can use any of the supported Google https://developers.google.com/workspace/drive/api/guides/ref-search-terms[search query terms and operators].
...
* *Success* 
+
String indicating if the search was successful and returned no errors (`True`)
+
To confirm if the search returns any results, verify if the *Amount found matches* is higher than zero. The *Success* variable returns (`True`) even if there are no results in a successful search. 
