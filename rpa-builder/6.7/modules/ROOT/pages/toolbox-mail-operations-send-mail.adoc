...
characters are available. The body is sent as plain text. For HTML bodies, see <<html-body,Sending mail with an HTML body>>.
...
In the Message area, you can enter text under *Body*. The text is sent as plain text, not as HTML. You can link the content with *Insert Available Variables*, assuming you defined variables in advance either using the Activity Parameters or the Workflow Based Variables. Select the encoding to use between UTF-8, UTF-16, Shift-JIS, and EUC-JP. To send HTML, see <<html-body,Sending mail with an HTML body>>. 
