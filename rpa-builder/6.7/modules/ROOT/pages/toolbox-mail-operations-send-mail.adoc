...
. In the script, use `System.Net.Mail.MailMessage` and set `IsBodyHtml` to `$true`. Configure SMTP and credentials to match your mail server and security policies.
...
In the Message area, enter text under *Body*. RPA Builder sends the content as plain text, not as HTML.   If you defined variables in advance either using the Activity Parameters or the Workflow Based Variables, link the content with *Insert Available Variables*. Select the encoding to use between UTF-8, UTF-16, Shift-JIS, and EUC-JP. To send HTML, see <<html-body,Sending mail with an HTML body>>. 
