...
The _Search Pattern_ action step searches for one or more patterns
...
subsequent action steps, for example as a reference point for mouse
clicks.
...
either until it is successful or the action step times out. A value of
0 means that the position of the pattern found is not rechecked after
having been found. The action step does not then wait beyond its
runtime.
...
All patterns captured using the _Search Pattern_ action step are
...
they can be added to the list of patterns for the action step using
...
included in another category of the same action step do not show.
...
You can use the _Search Pattern_ action step to check that a
