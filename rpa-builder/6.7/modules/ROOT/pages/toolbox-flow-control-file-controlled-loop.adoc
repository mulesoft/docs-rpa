...
The *File Content Preview* section enables you to apply filters to each column by clicking the *Filter* button (image:filter-icon.png["", 2%, 2%]). When you enter text in the *Filter* field, the preview filters out all rows that do not contain the specified text.
