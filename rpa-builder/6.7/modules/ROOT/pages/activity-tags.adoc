...
Select an Activity tag and click (image:add-tag.png["", 1%, 1%]) to add it to the workflow, or click (image:remove-tag.png["", 1%, 1%]) to remove the tag from the workflow. 

Use (image:add-all-tags.png["", 2%, 2%]) to add all tags to the workflow or (image:remove-all-tags.png["", 2%, 2%]) to remove all tags from the workflow.
