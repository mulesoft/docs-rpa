...
* (image:back-icon.png["", 2%, 2%]): Navigates back.
* (image:forward-icon.png["", 2%, 2%]): Navigates forward.
* (image:reload-icon.png["", 2%, 2%]): Reloads the current page.
* (image:clear-cache-cookies.png["", 2%, 2%]): Clears the browser cache data and Cookies.
* (image:double-check-icon.png["", 2%, 2%]): Confirms the selection and extracts the _XPath_ of the selected element along with the current input as values. This option is available only in the *Keystrokes to Web Element* Wizard.
* (image:check-icon.png["", 2%, 2%]): Confirms the selection and extracts the _XPath_ of the selected element.
