...
A Workflow can have one _Activity Parameters_ action step and it's automatically placed in the _Workflow Initialization_ section when creating a new Workflow.
...
. In the BPMN editor, click the *Business Process Initialization* (image:business-process-initialization-icon.png["", 2%, 2%]) button.
. Click *Activity Parameters*.
+
image:toolbox-activity-parameters-config.png[The Activity Parameters config window, 30%, 30%]
. In the Business Process Initialization window, click (image:add-icon.png["", 2%, 2%] *Add New Parameter ...*).
+
...
* Google API Service Account Credentials (*Google API Service Account Credentials*)
...
To delete an Activity Parameter, click the *Delete* (image:delete-icon.png["", 2%, 2%]) button next to the Activity Parameter you want to delete.

image:business-process-initialization-window.png[The business process initialization window, 75%, 75%]

You can delete an Activity Parameter if it's not being used in a Workflow. If the Activity Parameter is connected to at least one Workflow, a pop-up message indicates that the parameter is still in use.
...
. Click the *Edit Variable Mappings* (image:pin-icon.png["", 2%, 2%]) button next to the action step for which you want to connect an Activity Parameter.
+
...
. In the menu that opens, click the *Set Condition Expression* (image:cogs-icon.png["", 2%, 2%]) button.
+
