...
(and may also be used for all concurrent secure sessions).
+
To ensure optimal operation when using secure sessions, set it to `0`.
In this case, the port used is calculated by the following function:
`port = TSID * 4 + 45000` (TSID is the current Terminal Session ID). To
...
image:toolbox-variable-web-automation-web-session-images/image6.png[""]
...
* xref:rpa-bot::configuration.adoc#secure-session[Secure Session]