...
* (image:paste-icon.png["", 2%, 2%]) <<copy-paste, Paste copied object(s)>>
* (image:arrow-rotate-left-icon.png["", 2%, 2%]) <<undo-redo, Undo previous action>>
* (image:arrow-rotate-right-icon.png["", 2%, 2%]) <<undo-redo, Redo action>>
* (image:arrow-up.png["", 2%, 2%]) <<move-up-down, Move selected object up>>
* (image:arrow-down.png["", 2%, 2%]) <<move-up-down, Move selected object down>>
* (image:delete-icon-alt.png[The Delete selected object icon, 2%, 2%]) <<delete, Delete selected object(s)>>
* (image:run-wizard-icon.png["", 2%, 2%]) <<start-wizard, Run wizard for selected object>>
* (image:quick-creation-icon.png["", 2%, 2%]) <<start-quick-creation, Start Quick Creation Mode>>
* (image:validate-icon.png["", 2%, 2%]) <<validate, Validate Workflow>>
* (image:play-icon.png["", 2%, 2%]) <<run-workflow, Run Workflow>>
* (image:debug-icon.png["", 2%, 2%]) <<debug-workflow, Debug Workflow>>
* (image:print-icon.png["", 2%, 2%]) <<print-workflow, Print>>
...
Clicking *Copy* (image:copy-icon.png[The copy icon, 2%, 2%]) copies the selected Workflow objects and their underlying elements to the clipboard. Select the target object and click *Paste* (image:paste-icon.png["", 2%, 2%]) to paste it. You can also use _Ctrl + C_ for copying and _Ctrl + V_ for pasting.
...
To change the order of objects, select the object and move it up or down in the Workflow by clicking *Move up* (image:arrow-up.png["", 2%, 2%]) or *Move down* (image:arrow-down.png["", 2%, 2%]) until the object reaches the desired position. You can also drag and drop elements to rearrange them.
...
To configure a Workflow object with the wizard, select it and then click *Wizard* (image:run-wizard-icon.png["", 2%, 2%]). This process works for single Action Steps as well as for complete Transactions. You can also double-click a Workflow object to start its wizard.

[[start-quick-creation]]
== Start Quick Creation Mode

Use this function to speed up the initial Workflow creation. When you start a new Workflow project from scratch, click *Start Quick Creation Mode* (image:quick-creation-icon.png["", 2%, 2%]) to open the Quick Creation Mode interface.
...
If the Quick Creation Mode interface is blocking parts of the application you need to use in the Workflow, click *Switch Monitor Side* (image:switch-monitor-side-icon.png["", 2%, 2%]) and the interface switches from the right to the left side of the screen and vice versa. After you finish creating the Workflow, close Quick Creation Mode by clicking *Exit Quick Creation Mode* (image:exit-quick-creation-mode.png["", 2%, 2%]). After you exit Quick Creation Mode, the standard RPA Builder GUI shows again, including the Workflow you just created.

[[validate]]
== Validate Workflow

To check if a Workflow is ready for script generation, press *Validation* (image:validate-icon.png["", 2%, 2%]). If the project contains Workflow objects that are not configured, are wrongly configured, or have errors, RPA Builder shows warnings or general messages as a list in the Validation result window. Double-click an entry in this list to navigate to the corresponding Workflow object.

[[run-workflow]]
== Run Workflow

To test the created Workflow, click *Run Workflow* (image:play-icon.png["", 2%, 2%]) to generate or update the script file from the currently active project. The script executes automatically after its creation.
...
(image:reload-workflow-icon.png["", 2%, 2%]): Reverts back to the last saved state of your project.
...
(image:save-as-template-icon.png["", 2%, 2%]): To create a template, select the Workflow object you want to save as a template and click Save as template in the context menu. See xref:managing-user-templates.adoc[Managing User Templates] for additional details about creating, importing, and exporting User Templates.

=== Open Trace Log

(image:open-trace-log.png["", 2%, 2%]): Shows the content of the log file created during the last test run of this Workflow. Use this option to obtain additional information for failure analysis.

=== Load Analysis Package

(image:load-analysis-package.png["", 2%, 2%]): RPA Manager provides analysis packages for Workflow runs that don't execute successfully. RPA Builder can display errors that occur within a Workflow run, whether from a run started by RPA Builder or by reading the analysis package of a run executed in RPA Manager.
...
(image:clear-run-results-icon.png["", 2%, 2%]): Use this menu item to clear the markings of warnings and errors in the Workbench view of the selected project. These markings are shown after a failed local Workflow run or after the import of an analysis package.
...
At the end of the Workbench is the *Search...* (image:search-icon.png["", 2%, 2%]) function. If you gave your Workflow elements descriptive names and want to detect them in a Workflow, type the name (or part of it) into the search field and press Enter. The first element with a matching name shows on the Workbench. If there are additional matching occurrences, you can access them by pressing _F3_.
