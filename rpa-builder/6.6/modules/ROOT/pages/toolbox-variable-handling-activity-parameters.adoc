...
After you have added all necessary condition expressions, ensure that the last Sequence Flow has an empty condition expression. This is the default flow when all other conditions aren't fullfilled. If there is no default sequence flow, add one to the model.
