...
* [#AS_CreateFile_P_DirectoryPath .anchor]####**Directory path** This is
where you enter the path to the directory where the file is to be
...
* [#AS_CreateFile_P_FileName .anchor]####**File Name** Enter the name of
the file that is to be created here.
...
link:#AS_CreateFile_P_DirectoryPath[*Directory path*]

link:#AS_CreateFile_P_FileName[*File name*]
...
value is %STWS_BASEPATH%. If you specify an absolute path and also
...
existing path using the image:media\image3.png[image,width=20,height=20]
button. You can also insert variables from the _Workflow_ in order to
...
the section link:#AS_Settings[Settings].
