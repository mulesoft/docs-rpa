...
Tip: If you intend to use an _Image search_ within the application when
it is running, it is recommended that you use _Maximized_ _Start mode_
so the application window will always be in the same position! This
makes defining a reliable _Search area_ for the _Image search_ much
easier.
...
installed. So, if you put the following entry in the _File Name_ field
...
_X1 Design Studio_ is usually run as a 64-bit program but the generated
