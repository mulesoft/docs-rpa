...
link:#AS_Settings[Settings] chapter.

=== Properties

*File Operation Settings*

* [#AS_FileCheck_P_DirectoryPath .anchor]####**Directory path** Path to
...
* [#AS_FileCheck_P_FileCheckMode .anchor]####**File Check mode** Here,
...
* [#AS_FileCheck_P_FileName .anchor]####**File Name** Here, you can
enter the name of the file you are searching for.

Placeholders are not allowed. For further instructions, please observe
the section link:#AS_Settings_InvalidCharacters[Invalid Characters].

You will find further properties, which are the same for all Action
Steps, in the chapter link:#CommonProperties[Common Properties]. These
are not detailed here for the sake of clarity.

=== Inbound Variables

link:#AS_FileCheck_P_DirectoryPath[*Directory path*]

link:#AS_FileCheck_P_FileName[*File Name*]
...
or a path relative to the _Base Path_ using the
image:media\image1.png[image,width=175,height=22] and
image:media\image2.png[image,width=130,height=22] buttons. The initial
value is %STWS_BASEPATH%. You select an existing path using the
image:media\image3.png[image,width=20,height=20] button. You can also
...
the link:#AS_Settings[Settings] chapter.

In the link:#AS_FileCheck_P_FileName[File name] field, enter the name of
...
the available variables. You then use the
link:#AS_FileCheck_P_FileCheckMode[Check mode] to define the result that
