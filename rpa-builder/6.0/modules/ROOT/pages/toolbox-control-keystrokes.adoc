...
the capture)
* *Hide main window when start* Hides the X1 Design Studio window when
_Start_ is clicked
...
administrator privileges, the _X1 Design Studio_ has to be started with
...
Because _Servicetrace_ starts with a capital _S_, which involves the
keystrokes _SHIFT_ + _s_ , this _S_ has two _Delays_.
