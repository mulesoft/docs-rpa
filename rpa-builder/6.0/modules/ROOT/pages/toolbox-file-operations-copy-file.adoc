...
* [#AS_CopyFile_P_DestinationDirectory .anchor]####**Destination
directory** Enter the path to the target directory where the copied file
...
variable.
* [#AS_CopyFile_P_DirectoryPath .anchor]####**Directory path** This is
...
* [#AS_CopyFile_P_FileName .anchor]####**File Name** Enter the name of
...
link:#AS_CopyFile_P_DirectoryPath[*Directory path*]

link:#AS_CopyFile_P_FileName[*File name*]

link:#AS_CopyFile_P_DestinationDirectory[*Destination directory*]
...
be found under link:#AS_Settings[Settings].

* *Directory path* Specify the path to the file that is to be copied
here. You can create an absolute path or a path relative to the _Base
Path_ using the image:media\image1.png[image,width=175,height=22] and
image:media\image2.png[image,width=129,height=22] buttons. The initial
value is %STWS_BASEPATH%. Select an existing path using the
