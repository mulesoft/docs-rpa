...
generierte Beschreibung,width=290,height=125]
...
*Web Session ID* The web session ID identifies the web session to be
closed.

link:#AS_CloseWebSession_P_ForceErrorState[*Force error state if session
could not be closed*]

=== Outbound Variables

*Web Session closed* Indicates whether or not the web session was
closed.
