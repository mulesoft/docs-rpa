...
* {blank}
+
*X1 Server*: During the Design phase on the X1 Server, you have the
option to record a process with a sequence of clicks and keyboard
inputs. You can download the necessary Servicetrace Process Recorder
directly there (for more information, see the X1 server online manual).
Once you have finished the recording, you can upload it to the server,
together with the generated process, and move the process to the Build
phase. You can then download and open the process in the Design Studio.
All the recorded information for generating the Action Steps
automatically is now available.

* {blank}
+
*X1 Design Studio*: If the _App Session_ does not contain any elements
