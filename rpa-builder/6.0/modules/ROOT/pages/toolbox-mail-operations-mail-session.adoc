...
[#AS_MailSession_P_ServerAddress .anchor]####**Server address** (String)
Here, you see the server address that is entered in the wizard.

[#AS_MailSession_P_ServerPort .anchor]####**Server Port** (Integer,
Default: 143) Here, you see the selected port.
...
[#AS_MailSession_P_SMTPServerAddress .anchor]####**SMTP Server address**
(String) Here, you see the URL of the selected SMTP mail server.

*SMTP Server Port* (Integer) Displays the selected port.

[#AS_MailSession_P_UseDifferentCredentials .anchor]####**Use Different
Credentials** (Boolean) If the checkmark is set here, you can set
...
*link:\l[SMTP Server
port]link:#AS_MailSession_P_UseDifferentCredentials[Use Different
Credentials]*
...
The wizard is split into following sections: *Operation Mode*, *Incoming
...
Please note: If you have selected the _Send mails via SMTP_ mode, no
Read mail Action Step can be used in the Mail Session. This is also true
if you have selected the _Read mails from inbox_ mode. In this case, no
Send Mail Action Step can be used.

In the Incoming Mail area, you can make all the settings necessary to
import e-mails from an e-mail server. Here, you can also find the *Test
Connection Button*. You can use this to test whether the connection to
...
Using the pin image:media\image1.png[image,width=16,height=21], you can
also link the buttons to variables.
