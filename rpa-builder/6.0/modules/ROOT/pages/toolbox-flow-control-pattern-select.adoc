...
*link:#AS_PatternSelect_OV_FindCoordinate[Find Coordinate]*
...
[#AS_PatternSelect_OV_FindCoordinate .anchor]####**Find Coordinate** Top
...
pattern operations, is described in the section
link:#PatternStrip[Pattern Strip].
...
image:media\image3.png[Ein Bild, das Tisch enthält. Automatisch
generierte Beschreibung,width=276,height=97]
...
generierte Beschreibung,width=422,height=151]
...
Steps as the Workflow progresses (see link:#_Other_Uses_of[Other Uses of
Matched Patterns]).
...
link:#CapturingTheSearchPattern[Capturing the Search Pattern].
