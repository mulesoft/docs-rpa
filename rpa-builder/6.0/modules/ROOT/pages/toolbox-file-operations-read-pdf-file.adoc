...
[#AS_ReadPDFFile_P_DirectoryPath .anchor]####**Directory Path** (String)
The path to the folder in which the PDF file is located.

[#AS_ReadPDFFile_P_FileName .anchor]####**File Name** (String) The name
of the PDF file.

[#AS_ReadPDFFile_P_PasswordProtected .anchor]####**Password protected**
(Boolean) This indicates whether the PDF document is to be opened with a
password.
...
+
____
[#AS_ReadPDFFile_P_ReadTextFromPage .anchor]####**Read text from page**
(Integer) The text is read from this page onward.
____
* {blank}
+
____
[#AS_ReadPDFFile_P_ReadTextToPage .anchor]####**Read text to page**
...
file is located and the name of the file. If the PDF is password
protected, select the option
image:media\image1.png[image,width=166,height=20]. You can now enter the
password directly as free text or select it as a variable using the pin
icon.
...
generierte Beschreibung,width=364,height=256]
