...
You can start _X1 Design Studio_ by clicking the desktop icon or from
the Windows start menu.

The _X1 Design Studio_ consists of the main areas menu bar, Business
Process Modeler, Workbench, left window pane, right window pane and
Workflow Run Results:

image::getting-started-installation-and-first-use-application-overview-image1.png[Overview window panes]

In addition to the Workflow Run Result, you will also find the Process
Status in the lower area.

image::getting-started-installation-and-first-use-application-overview-image2.png[Process Status pane]

Most visual elements in the _X1 Design Studio_ have tooltips. If you
ever want to know what a button or setting does, simply hover your mouse
pointer over it for a few seconds.

=== Menu Bar

In the menu bar, you will find icons for saving, uploading, and
validating projects. The other icons are shortcuts to various views in
the _X1 Design Studio_.

=== Tools

*Open X1 Design Studio Log* This menu item opens the X1 Design Studio
log file in your default text editor. The log is a text file containing
a chronological list of all relevant events since X1 Design Studio was
...
image::getting-started-installation-and-first-use-application-overview-image4.png[Option General]
...
link:#options[options] and _Page Setup…_ und _Print Template Settings_
for the link:#_The__Workflow[Workflow Run Protocol] and
link:#_Print[Printing] of a Workflow are saved into the folder
C:\Users\Public\Documents\ServiceTracerCommon\X1. Otherwise these
settings are saved to the folder \Documents
C:\Users\[UserName.Domain]\Documents\ Servicetrace\X1 of the respective
user.

*Open last project on start* Selecting this option means that the last
project that you opened will be reloaded and displayed when you launch
the X1 Design Studio.
...
This setting affects *all* X1 Design Studio users.

image::getting-started-installation-and-first-use-application-overview-image5.png[Option Autosave]

*Autosave on script run*

If this option is selected, the Workflow will be saved automatically
before each local run.

*Common Autosave*

If this option is activated, projects will be saved automatically at
intervals of a few minutes.

It will be always saved the entire Business Process, including all
Workflows.

image::getting-started-installation-and-first-use-application-overview-image6.png[Option Repository]

You can find a detailed description of the _Repository Settings_ in the
chapter link:#_Configuring_the_WorkflowEditor[Configuring the X1 Design
Studio].

image::getting-started-installation-and-first-use-application-overview-image7.png[Option Visibility]
...
_Protocol Preview_ of the link:#_The__Workflow[_Workflow Run Protocols_]
will be opened directly after the local run of the Workflow in the X1
Design Studio.

*Show system parameters dialog*

If this option is set, the system parameters will be shown whenever a
new Activity Workflow is created. These can also be shown via the menu
_View_ -> _Show System Parameters_.

*Plug-ins* All plug-ins will have been installed automatically. For
...
containing the views _Toolbox_, _Project,_ Activity Library,
...
You can find a detailed description for each element in chapter
link:#TheToolbox[The Toolbox].

If you input text in the field _Filter_ the content of the _Toolbox_
will be reduced successively to elements, the names of which contain
this text.
