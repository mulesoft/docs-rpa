...
In the *Operation mode* _Mouse action_, you determine the type of the
action (such as Double Left Click), while _Trigger Count_ defines the
number of times this action is performed. The Action Step ensures that
the clicks are grouped correctly. In other words, a configured
double-click is executed directly, while a normal Left Click with two
repetitions is executed with a time delay, to ensure that Windows does
not register a double-click.
