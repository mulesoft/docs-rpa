...
Use the link:#AS_SetVariable[Set Variable] and the
link:#_Set_Variable[Set Array Variable] Action Steps to overwrite the
initial variable values with new values at a later point in the
Workflow.

The _Workflow Based Variables_ Action Step can only appear once in a
Workflow and must be positioned in the _Workflow Initialization_
section.

=== Properties

This Action Step contains no special properties other than the
link:#CommonProperties[common] ones.
...
generierte Beschreibung,width=271,height=132]
...
generierte Beschreibung,width=265,height=103]
