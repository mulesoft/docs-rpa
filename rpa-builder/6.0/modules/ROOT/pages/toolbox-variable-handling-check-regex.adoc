...
An introduction to regular expressions is provided in the section on
link:#UsingRegularExpression[Using Regular Expressions].

=== Properties

*Check Regex Settings*

* [#AS_CheckRegex_P_InputText .anchor]####**Input text** Enter the text
that the regular expression is to be applied to. This property is only
displayed if no variable has been specified in the _Check Regex_ wizard
as the target of the regular expression search.
* [#AS_CheckRegex_P_RegexPattern .anchor]####**Regex Pattern** Enter the
regular expression that you want to use as the basis of the search.
...
assign a variable. Click the
image:media\image1.png[image,width=21,height=20] button to open a dialog
for testing the regular expression. The regular expression already
specified in the *Regex* field will appear automatically in the _Regex
Tester_ dialog.
