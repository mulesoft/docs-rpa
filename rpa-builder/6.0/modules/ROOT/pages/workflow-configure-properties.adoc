...
. Right-click the Workflow and select *Edit Workflow Properties* from the context menu.
+
image:workflow-properties-window.png[The Project Explorer showing the Workflow to edit, 50%, 50%]
+
The corresponding properties are then displayed on the properties panel of RPA Builder.
...
** *Color Depth*
+
Compensate for color depth variations of different measurement robots by using the Automatic mode.
...
+
Process names entered into this field must contain no spaces and be separated in the list by semicolons as in this example: `proc_1.exe;proc_2.exe;proc_N.exe`.
** *Keep Alive*
+
A list of all processes that STC Guard that are not ended if an error occurs while executing a Workflow.
+
Process names entered into this field must contain no spaces and be separated in the list by semicolons as in this example: `proc_1.exe;proc_2.exe;proc_N.exe`.
+
[NOTE]
...
** *Last Editor*
+
Microsoft Windows® Login ID of the last person to change the project (automatically set by the program).
** *Workflow Type*
+
