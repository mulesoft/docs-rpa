...
link:#AS_FileControlledLoop[_File Controlled Loop_] and also forms the
...
session. You enter the source file in the link:#AS_ExcelSession[Excel
Session].
...
link:\l[*Regex to match*]

==== link:\l[*Regex condition*]Outbound Variables
...
Two settings are particularly important, *Read mode* and *Terminate
loop*.
...
reading is to begin from but also the point that reading should
terminate at.
+
image:media\image2.png[image,width=311,height=69]
* *Read to row* Here, you can specify the row that the spreadsheet is to
be read to.
+
image:media\image3.png[image,width=320,height=62]
* *Read to column* Here, you can specify the column that the spreadsheet
is to be read to.
+
image:media\image4.png[Ein Bild, das Text enthält. Automatisch
generierte Beschreibung,width=369,height=60]
...
generierte Beschreibung,width=440,height=48]
...
generierte Beschreibung,width=445,height=52]
* *Only read a maximum number of rows* Here, you can specify the maximum
...
generierte Beschreibung,width=528,height=236]

However, if you have selected the *Has a header* option in the wizard,
the first row is used as the column header.

image:media\image11.png[Ein Bild, das Text enthält. Automatisch
generierte Beschreibung,width=528,height=227]

The data type of each column is recognized automatically. However, it is
easy to change.
