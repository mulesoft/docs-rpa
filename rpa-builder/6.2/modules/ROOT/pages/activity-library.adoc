...
A particular case is the Start event (Trigger). Although it is not an activity in the strict sense, a workflow is still assigned to this item, which initially starts the process. You can upload this event to the Activity Library. 

The Activity Library Explorer enables you to edit, copy, or delete activities and search for activities in your organization's library based on Activity Tags, which you can create and assign to the activities to further categorization and filtering capabilities. 
...
+
The Activity Library Explorer shows only the appropriate types of activities you can import, depending on which element you selected in the workflow. 
