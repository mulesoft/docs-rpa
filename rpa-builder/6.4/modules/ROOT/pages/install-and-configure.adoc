...
+
image:anypoint-login.png[The Anypoint Platform login screen prompt to configure the login credentials]
...
== Changing the Log-in Credentials

To connect to a different RPA Manager instance or change the configured login credentials:

. In the RPA Builder menu bar, navigate to *Options > Account*.
. Click *Sign out*.
