...
* If you want to run more than one (Windows 10, Windows 11) or two (Windows Server) Secure Session in parallel on a bot, make sure that a terminal server software is installed on the bot's computer with enough resources to execute several processes simultaneously (2 GB or more RAM per session). 
