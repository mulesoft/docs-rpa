...
// question: correct requirements?
** If you want to use another account, ensure that it has permission to run Windows services.
// question: local account vs domain account?
* The RPA Bot service runs processes under user accounts existing on the same computer or within the same domain.
// question: Is it enough that the bot users belong to the _Remote Desktop Users_ group or is only the port 127.0.0.2 allowed? Do we need to mention this port specificially?
...
** User accounts used by the bot to run processes must belong to the _Remote Desktop Users_ group and must be allowed to establish a local RDP session via the port 127.0.0.2.
