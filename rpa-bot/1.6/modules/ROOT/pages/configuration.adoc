...
The diagnostic panel shows important runtime information to identify current and potential problems with RPA Bot. The panel also shows which Windows session is currently being used by which secure session. Some values cannot be determined when RPA Bot is not running.
...
When changes are made to the RPA Bot Configuring while a recording is in progress, 
