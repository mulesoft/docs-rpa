...
+
For example, if you want to run 10 processes in parallel, that require 1 GB RAM each, you need a computer with a minimum total RAM of 38 GB:
+
[stem]
++++
RAM_{"Total"} = RAM_{"Minimum"} + (RAM_{"Bot"} * Number_{"Parallel Sessions"}) + (RAM_{"Process"} * Number_{"Parallel Sessions"}) = 8 + (10 * 2) + (10 * 1) = 38
++++
