...
* The following table shows the minimum and the recommended hardware requirements for running one process. To run more than one process in parallel, when using Secure Sesions, you need to add 2 GB of RAM plus the RAM needed to run the process for each parallel run to these recommended values.
+
