...
To prevent issues while executing the Secure Session: 

* <<create-local-user, Create a local user for the Secure Session>>
+
Creating a local user for the bot to use prevents receiving errors such as _Insufficient Privilege_, _Conflicting Username_, or _Wrong Credentials_. Using a local user speeds up the authentication process and terminal session setup. 

If your security policies do not allow local users or the applications to use during the automation require SSO authentication via a domain user, use a domain user. 
...
+
** Administrators (required only when the applications to use require Administrator rights)
