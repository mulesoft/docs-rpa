...
+
If you want the bot to use Secure Sessions, enter the name of the user the bot uses to log in to the computer and to run a hidden session. If the environment of the computer the bot is installed on is embedded in a domain and you want to use a local user, write the user name as follows: `.\username`.
...
+
There is a Group Policy called `fPromptForPassword`. If this policy is enabled, you must enter the password a second time, when the RDP session is established. If this is the case, enable this option. Also, choose the correct form of dialog. Some machines request the second password entry via a popup dialog. Other machines embedd the prompt into the same screen, like the regular Windows login.
* *Handle a “Custom LogonMessage”*
+
It is possible to configure a Custom Logon Message screen that is displayed after the login into an RDP session. It generally needs to be acknowledged by pressing a button. If you are prompted with this message when connecting to your system via RDP, enable this option.
...
If you click *TEST*, the application starts a non-operational Secure Session in which you can see your parameters for the Secure Session listed. Click *Logout* to end this non-operational Secure Session.
...
* *Connection settings*
+
Here you can define how RPA Bot components (Robot, StSec and StSem) communicate among themselves. These settings are only relevant for Secure Sessions. You can choose the Named Pipes protocol or TCP. Named Pipes is selected by default, as an uncomplicated method. In some cases, when using TCP, the firewall can block the connection even though it is an exclusive internal computer process.
