...
* Dynamic process execution throttling is added to reduce the number of times RPA Bot attempts to restart a failing process.
* You can now view the path to the analysis packages on the default run result message in RPA Manager if the setting to write an analysis package is enabled on RPA Bot.
