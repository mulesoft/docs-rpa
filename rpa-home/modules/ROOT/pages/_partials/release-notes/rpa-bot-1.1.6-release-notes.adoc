...
|===
| Issue Resolution | ID
| RPA Bot now successfully executes all Invocable processes, no longer causing process instances to stop working on RPA Manager. | W-12250345
...
| RPA Bot no longer causes an error when RPA Manager tries to restart an invocable process that filed its execution on the bot. | W-11680817
|===
