= RPA Builder 6.5.8 Release Notes
...
|===
| Issue Resolution | ID
| The REST Call action step now returns the _Status_ variable even when the result is unsuccessful. | W-15038359
| Workflows no longer abort execution with error `ServicetraceWriteFileTest.txt is being used by another process`. | W-15004317
|===
