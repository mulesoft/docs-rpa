...
* The *Retake* option is now a button within a dropdown that includes options to delay the screen freeze for capturing image-based mouse. This assists with preparing collapsible menus and hidden elements before the screen capture freeze. User and mouse actions are not recorded during the delay.
...
* The attribute selection panel can no longer be opened when recording image-based mouse clicks in a Web Task. Once image-based mouse clicks recording is finished, the panel can be opened.
* Upon completion of recording image-based mouse clicks for a Web or Bot Task, a message now appears to indicate that recording has been resumed.
...
* Starting Recorder with Remote Debugging port errors.
...
|The `Page not available` error now displays when navigating back and forward in a browser while recording a Web Task. |W-12481508
|===
// == See Also
