...
* The *Retake* option is now a button in a dropdown that includes options to delay the screen freeze for capturing image-based mouse clicks. This assists with preparing collapsible menus and hidden elements before the screen capture freeze. User and mouse actions are not recorded during the delay.
...
* Updated the UI of the Settings and Log files panels, as well as the Manual Activity and Comment dialogs. W-14321940
...
* Starting RPA Recorder with Remote Debugging port no longer returns an error.
