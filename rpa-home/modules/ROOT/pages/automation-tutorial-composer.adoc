...
. Click *Add* (image:add-icon.png["", 2%, 2%]) next to the Scheduler event you previously created. 
...
. In *RPA Process*, select the invocable run configuration that you created in xref:automation-tutorial-deploy.adoc#Step-7-1[Step 7.1: Create an Invocable Run Configuration]. 
