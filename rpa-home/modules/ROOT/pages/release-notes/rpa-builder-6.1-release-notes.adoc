...
* The new Credentials for OAuth Action Step enables you to configure OAuth credentials once and reuse them within other Action Steps.
...
// This seems like a pre-release fix, do we need to include it? | |W-11887027 Send test mail with Exchange
// This seems like a pre-release fix, do we need to include it?| |W-11918594 OutlookMailSession contains previous data
// This seems like a pre-release fix, do we need to include it?| |W-11968720 Fixed Errorhandling None not working on various Control Blocks
| The Message Box Action Step Wizard now validates that the Title and Text values are not empty. |W-11864908
...
// This one can be shown in What's new as an update | |W-12041665 Removed Online Manual Selection in Settings Dialog
// This one can be shown in What's new as an update | |W-12041695 renamed "use all user folder" setting
| The Message Box Action Step no longer fails to recognize a normalized JSON from an AWS Action Step as a pin variable.  |W-12065095
// | |W-12065550
// | |W-11508834
|===
