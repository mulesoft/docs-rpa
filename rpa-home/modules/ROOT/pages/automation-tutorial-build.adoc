...
. In RPA Builder, drag the *Create bot task* element (image:create-bot-task-icon.png["", 2%, 2%]) from the BPMN editor into your process flow, after the *Sign-up to newsletter* task.
+
...
. In *Search Text*, write `"Thanks\s*for\s*signing\s*up"`.
+
This regular expression matches the text even if it spans over multiple lines. 
. Click *OK*.
...
. In the BPMN editor, click the *Run Process* (image:run-process.png["",1.5%,1.5%]) button.
+
...
+
As the tasks start executing, notice that new entries appear in this window and show a *Task execution succeeded* (image:task-execution-icon.png["", 2%, 2%]) icon in the *Run state* column when they finish.
+
[IMPORTANT]
