...
+
To access MuleSoft Intelligent Document Processing (IDP) with the RPA Builder xref:rpa-builder::toolbox-mulesoft-idp.adoc[MuleSoft IDP] action steps.
* *OAuth Credentials*
+
...
+
[IMPORTANT]
In the current version of the *OAuth Credentials* *OAuth Host*, is replaced with *OAuth Authorization Endpoint* and *OAuth Token Endpoint*. Check your processes to see if they have exchanged the Activity Parameters and create new credentials to link.
+
*** *OAuth Host*
+
