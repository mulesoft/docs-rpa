...
You can create multiple overlapping schedules. All schedules together make up a time frame within which the process executes. The executing bot checks every minute whether it's within the time frame. A new process run doesn't start until all conditions are fulfilled.
