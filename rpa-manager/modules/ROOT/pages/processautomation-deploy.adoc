...
+
The intervals are equidistant in the defined time frame. For example, `Repeat my process every 10 minutes`. If you need a more complex interval, use a Cron expression.
+
** *Cron expression*
+
Enter a Cron expression to repeat the process. The process starts once at the beginning of each interval.
+
The intervals don't need to be equidistant. For example, `Repeat my process every minute, between 12:00 PM and 12:59 PM, only on Monday and Friday, only in February, April, and June`.
+
...
[%collapsible]
=====
Cron is a widely used standard for describing time and date information. You use a cron expression to define complex intervals, for example, 'Run my process on 2:34 AM every third Friday of every second month.'.

The scheduler keeps track of every second and starts the process run when the Quartz Cron expression matches the bot's time-date setting. You can trigger the event
