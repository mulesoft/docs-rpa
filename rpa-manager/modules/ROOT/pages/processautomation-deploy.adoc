...
+
The productive run is carried out continuously within a fixed schedule or just once on the click of a trigger button. 
+
[[run-once-tag]]If you want to run the process only once on each <<trigger-run-once, click of the trigger button>>, select *Manual Execution*.
+
If you want to schedule the process run, consider and do the following:
+
For each schedule, you can define time intervals within which the process is executed.
+
...
The configuration to run once must either be invocable or marked as <<run-once-tag, *Manual Execution*>>.
...
. In the window *Select Configuration to Manual execution*, check all configurations you want to run.
. Click *Execute*.
