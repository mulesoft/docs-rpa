...
* To run a process in an existing console or RDP session, you must deploy it to a bot that is configured for console sessions and the specified user must be logged in in this session when you deploy the run configuration. If the user is logged in more than once, the bot starts the process in the first session it finds the user logged in to. The bot searches for the user in the order real console session, RDP session 1 to n. Do not close or minimize the session while the bot executes the process.
* To run a process in a hidden session, you must deploy it to a bot configured for secure sessions. Before deploying a run configuration on a bot that has only one secure session licensed, do not disconnect the session, but sign out the specified user. Do not sign in to a computer where a bot with one licensed secure session is running a process.
...
Alternatively, you can set the value of an activity parameter by linking it with a global variable. To link the parameter to a variable, click the *Link a global activity parameter* icon image:linkglobalvariable-icon.png[symbol,1.5%,1.5%] and select a variable. RPA Manager displays variables of the same type, assigned to the test phase, and only the credentials to which you are xref:processautomation-prepare-deployment-credential.adoc#form-create-credential-useraffiliation[affiliated]. If a credential affiliated to another user is already linked, RPA Manager displays a row of stars to hide the data. 
+
The variables are typed. You can only enter or link values of the same type. The values entered here are only start values and they can be overwritten by other values during the course of the process run.
* *User Task*
+
...
* *Execution*
+
Decide whether the test run is to be carried out in an existing console or RDP session or in a secure session, and specify an appropriate user.
+
You can link the user with a global credential variable. To do this,  click on the *Link a credential from credential pool* icon.
+
...
+
Make sure the bot you want to execute the process is configured for the execution type, because you cannot save the test configuration otherwise.
+
The console or RDP session can only be overtaken by the bot if the specified user is logged in at run time. If no user or another user is logged in at run time, the process run results show the failure *No active session with the user '<user name>' could be found on the system*. 
+
...
* *Execution*:
+
Decide whether the productive run is to be carried out in an existing console or RDP session or in a secure session, and specify an appropriate user.
+
You can link the user with a global credential variable. To do this,  click on the *Link a credential from credential pool* icon.
+
Use the syntax `domain\username` instead of `username@domain.com` for domain users and `username` for local users, regardless of whether you enter the username manually or link a global credential.
+
Make sure the bot you want to execute the process is configured for the execution type, because you cannot save the run configuration otherwise.
+
The console or RDP session can only be overtaken by the bot if the specified user is logged in at run time. If no user or another user is logged in at run time, the process run results show the failure *No active session with the user '<user name>' could be found on the system*. 
+
...
* *Execution*:
+
Decide whether the invokable run is to be carried out in an existing console or RDP session or in a secure session, and specify an appropriate user.
+
You can link the user with a global credential variable. To do this,  click on the *Link a credential from credential pool* icon.
+
Use the syntax `domain\username` instead of `username@domain.com` for domain users and `username` for local users, regardless of whether you enter the username manually or link a global credential.
