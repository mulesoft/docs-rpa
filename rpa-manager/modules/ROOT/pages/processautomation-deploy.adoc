...
The final phase in the life cycle of a process is the *Production* phase. This phase is structured similarly to the *Test* phase. You create a process run configuration which is then executed at the click of a button or automatically at scheduled times. Or you create an invokable run configuration and publish it to Anypoint Exchange.
...
Sometimes it's necessary to start a process manually when something happens that can only be evaluated by a human. For example, the goods inwards inspection process can only be started once the banana delivery ship reaches the pier wall. For this purpose, use the *Manual Execution* option.
...
+
The productive run is carried out continuously within a fixed schedule or just once at the click of a trigger button. 
+
[[run-once-tag]]If you want to run the process once on each <<trigger-run-once, click of the trigger button>>, select *Manual Execution*.
+
If you want to schedule the process run, consider and do the following:
+
For each schedule, you can define time intervals within which the process is executed continuously.
+
...
The configuration to run once must either be invokable or marked as <<run-once-tag, *Manual Execution*>>.
...
. Click on the name of a project to open it.
. Click *Select action*.
. From the dropdown, select *Manual execution*.
. In the window *Select Configuration to Manual execution*, check all configurations to run.
. Click *Execute*.
