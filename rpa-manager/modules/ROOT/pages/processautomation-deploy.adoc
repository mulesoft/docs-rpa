...
Transition the fully built process to test phase for testing.
...
+
You can create multiple schedules. These may also overlap. All schedules together make up a time frame within which the process may be executed. The executing bot checks every minute whether it is within this time frame or not. A new process runs is not started until all conditions have been fulfilled.
+
...
* xref:usermanagment-manage.adoc[Managing Users]
* xref:rpa-builder::index.adoc[RPA Builder Overview]
* xref:rpa-bot::index.adoc[RPA Bot Overview]
* xref:botmanagement-overview.adoc[Bot Management Overview]
