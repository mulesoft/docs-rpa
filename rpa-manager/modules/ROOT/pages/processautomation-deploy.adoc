...
Sometimes it's necessary to start a process manually when something happens that can only be evaluated by a human. For example, the goods inwards inspection process can only be started once the banana delivery ship reaches the pier wall. For this purpose, use the *Start manually* option.
...
+
[[run-once-tag]]If you want to run the process once on each <<trigger-run-once, click of the trigger button>>, select *Start Manually*.
+
