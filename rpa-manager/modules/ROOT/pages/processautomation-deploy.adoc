...
* To run a process in an existing console or RDP session, you must deploy it to a bot that is configured for console sessions and the specified user must be logged in in this session when you deploy the run configuration.
* To run a process in a hidden session, you must deploy it to a bot configured for secure sessions.
