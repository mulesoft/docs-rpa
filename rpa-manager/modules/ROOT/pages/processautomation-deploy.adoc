...
** Be aware that bots might live in different time zones.
...
+
You create a schedule without specifying a time zone. The bot you assign the production configuration to will execute the process at the specified times in its local time. The time intervals may change depending on daylight saving time.
+
For example, if you work in Buenos Aires (ART (UTC -3)), specify a schedule starting at 5:30 AM and ending at 9:30 PM and assign the configuration to three bots installed on computers in Berlin, San Francisco, and Tokyo, the bots will execute the processes not simultaneously:
+
[cols="1,2,2"]
|===
