...
The example shows the planned process executions from the view of a user who lives in the Coordinated Universal timezone. All times are converted to the user's time zone.

The shown configuration uses a schedule which is repeated daily from 12 PM to 20 PM. The bot is installed on a computer using the same timezone as the user. It has an assigned service time defined in German time. The application is installed on a computer in Germany as well, which has a daily down time from 15:30 PM to 16:30 PM.
When schedule and service time overlap and all applications are active, the bot executes the process. In this example, a downtime takes place in the middle of the scheduled time. The execution is interrupted during that time. Because the service time ends before the scheduled run time, the effective runtime is shortened, too.
