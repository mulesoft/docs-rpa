...
If a bot is running properly, but the process execution fail, and you need help to detect the cause, turn on debug logging. 
...
[IMPORTANT]
Debug Logging is supported by RPA Bots with version 1.3.3 and above.

If a bot is running properly, but the process execution fail, and you need help to detect the cause, turn on debug logging for at least the time span of the next process run.
...
. In the modal *Enable time limited debug logging*, enter a time interval in minutes and click *Start debug logging*.

After the time interval has lapsed, the log level returns to the default.

If a users sets a new debug interval while the bot is still in debug mode, the interval starts anew with the new value. You cannot see if and for which interval the bot is in debug mode.
