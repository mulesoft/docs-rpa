...
The execution timetable shows you the effective runtime of all calendar scheduled run configurations assigned to the bot. The execution timetable does not show run configurations using other types of scheduling, such as on-demand, interval or cron-expression.
...
The bot runs a process when the bot's service time overlaps with the schedule of the configuration. If the process uses applications, the bot doesn't run the process during the downtimes of any of those applications.
