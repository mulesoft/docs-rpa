...
If a user sets a new debug interval while the bot is still in debug mode, the interval restarts with the new value. You cannot see if and for which interval the bot is in debug mode.
