...
The example shows the planned process executions from the view of a user who lives in the coordinated universal timezone. The shown configuration uses a schedule which is repeated daily from 12 PM to 20 PM. The bot is installed on a computer using the same timezone as the user. It has an asigned service time defined in German time. The application is installed on a computer in Germany as well, which has a daily down time from 15:30 PM to 16:30 PM.
When schedule and service time overlap, the process can be executed. In this example, a downtime takes place in the middle of the scheduled time. The execution is interrrupted during that time. Because the service time is shorter than the scheduled run time, the execution is shortened, too.
