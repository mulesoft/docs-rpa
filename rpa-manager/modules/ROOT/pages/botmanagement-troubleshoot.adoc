...
. Click the *Download Logfiles* icon image:download-icon.png[disk-with-downwards-arrow symbol,1.5%,1.5%]  in the table row of the bot you want to read the log files from.
...
Restarting a bot only works if that bot has the status OK. Otherwise RPA Manager places the restart in the task list and executes it soon as the bot is OK again.
