...
If a bot has status *STOPPED*, *OVERDUE*, or *UNKNOWN*, an IT administrator needs to log on the computer the bot is installed and troubleshoot it there.
...
Use this function to restart a bot instead of using the Task Manager on the computer where it is installed, to ensure that all the processes the bot is currently executing are completed properly before the bot is stopped.
...
You can tell this has occurred when the bot has the status OK and a deployed process is within an effective runtime interval, but no process has been started for several minutes.
