...
A task consists of a name, a description, an owner, a priority, a due date, and an effort estimate. You can drag tasks across the board to reflect their progress.

A process board is a table with three columns, across which you can arrange task items:

* *To-do*
+
The *To-do* column contains all the tasks that have not been processed yet. You create new tasks here. If you have <<createautomaticephasereleasetasks, enabled the creation of automatically generated tasks for phase transitions>>, RPA Manager creates a new corresponding task during each phase transition. Remember to move tasks that are being processed into the *In Progress* area.
* *In Progress*
+
...
. Open the *Project Management* view of the *Process Automation* module.
. Click on the name of the category of the project.
. In the *<category name>* view, click the *Open process board* icon image:processboard-icon.png[column symbol,1.5%,1.5%] on the project panel or in an already opened project.
...
* *Priority*:
+
You can set the priority of the task to low, medium, or high. The task item displays a grey, yellow or red label, respectively.
* *Due date*:
+
Date on which the task should be completed.
* *Effort*:
+
Time the task will approximately take.
. Click Save.
...
+
For an explanation of the properties, see <<form-createprocessboardtask, *Create new task*>>.
. Click *Save*
...
If you have enabled the creation of automatically generated tasks for the phase transitions, move this task to the *Completed* column after all other tasks have been completed to enable the transition of the project to the next phase.
