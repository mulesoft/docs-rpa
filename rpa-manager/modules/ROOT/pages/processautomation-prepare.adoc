...
Use decision tables to model business rules.
...
+
The properties are explained in the <<form-create-credential, *Create a New Credential*>> form.
. Click *Save*.
...
[[manage-decisionmodels]]
=== Manage Decision Models 

Decision models contain all the knowledge that is needed to make complex decisions. You model the interrelation of input parameters, knowledge sources, and decisions in decsion requirements diagrams (DRD). You model decisions by listing all possible combinations of input and output parameters in decision tables (DT).
...
+
Read the topic *Designing Decisions* to find out how to create an DRD.
. Click *Save*
