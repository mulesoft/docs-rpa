...
You can deploy a process on a bot which has assigned the required applications. Down times are repeatable time intervals that define when an application is not available. Processes whose applications are currently down are executed to completion and are not restarted until the downtime period has ended.
...
For example, if you specify a down time starting at 1am and ending at 5am in Tokyo time (JST (UTC +9)) and the bot is installed on a computer in San Francisco (PST (UTC -8)), the bot will not execute processes between 8am and 12pm in San Francisco time. If you assign the application to multiple bots in different time zones, all bots will pause executing processes using that application simultaneously.
