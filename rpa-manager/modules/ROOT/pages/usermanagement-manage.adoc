...
Reset a password if the user forgot it. RPA Manager will send an email containing a new temporary password to the user. The user can the create a new password.
...
Remove obsolete users. A user cannot be deleted if it is the owner or manager of a process. Deleted users cannot be restored. Use the *Deactivate* function to remove a user only temporary.
...
Assign users who will build the process automation RPA Builder access. Currently working users might not be able to save their work if their license gets revoked.
