...
To start RPA Recorder click the *Record BPMN* icon (image:record-bpmn-icon.png[Record BPMN icon, 25%, 25%]) in RPA Manager.
...
After starting the recorder via Record BPMN (image:record-bpmn-icon.png[Record BPMN icon, 25%, 25%]) in RPA Manager:

. Open the process recorder.
. Provide your login credentials for user authentication.

To log out of RPA Recorder, click the *Logout* icon (image:logout-icon.png[Logout icon, 25%, 25%]).
