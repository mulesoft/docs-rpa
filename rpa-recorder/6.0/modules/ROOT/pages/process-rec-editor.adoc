...
Select a process and click on the *Edit, Merge and Upload Recordings* button (image:edit-process-button.png[Edit, Merge and Upload Recordings button, 0.025%, 0.025%]) to open the Process Recording Editor.
...
After merging and editing the BPMN and documentation, you can upload the new process BMPN by clicking the *Upload to server* icon (image:upload-to-server-icon.png[Upload to Server icon, 2%, 2%]). A message opens confirming that the upload is completed.

You can then close the Process Recording Editor and the Process Recorder by clicking the *Close* icon (image:close-icon.png[Close icon, 2%, 2%]) in the top right corner.
