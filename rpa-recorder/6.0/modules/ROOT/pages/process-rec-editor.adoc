...
Select a process and click on the *Edit, Merge and Upload Recordings* button (image:edit-process-button.png[Edit, Merge and Upload Recordings button, 25%, 25%]) to open the Process Recording Editor.
...
. Click the *Load File* (image:load-file-button.png[Load File button, 25%, 25%]) button.
...
* After saving for the first time, you can click on the *Save* button (image:save-icon.png[Save changes button, 25%, 25%]) to save further changes to the selected filename and destination.

To open the *Save recording as* dialog box again, click the *Save as* button (image:save-icon-dark.png[Save As button, 25%, 25%]) to choose a new name, variant tag and saving destination.

== Upload BPMN Back to the Server

After merging and editing the BPMN and documentation, you can upload the new process BMPN by clicking the *Upload to server* icon (image:upoad-to-server-icon.png[Upload to Server icon, 25%, 25%]). A message opens confirming that the upload is completed.

You can then close the Process Recording Editor and the Process Recorder by clicking the *Close* icon (image:close-icon.png[Close icon, 25%, 25%]) in the top right corner.
