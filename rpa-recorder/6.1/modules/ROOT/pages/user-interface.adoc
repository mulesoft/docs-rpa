...
Use the Web Recording Browser to record website navigation actions. It behaves as a normal web browser and enables you to:

* Select additional elements of a website and determine their XPath and attributes.
...
Adding a web task opens the Web Recording Browser. If you choose to continue using the session of the last closed Web Task, the browser loads the URL that is defined as the initial URL for the enclosing Web Session.
Otherwise, you can enter a URL in the address bar of the browser, and it loads this URL when you press Enter or click the image:play-icon.png[Play button].

image:web-recording-browser-blank.png[Web Recording Browser]

=== Select Web Elements

When you hover the cursor over a web element, the browser highlights it with a blue background.
To select the highlighted element press `F2`. After selecting an element, its XPath and available attributes show on the Attribute Selection panel.

image:web-recording-select-element.png[Select Web Element]

By default, the Attribute Selection panel is hidden. Double-click on the right border of the browser to extend or collapse it.
...
Select one or more attributes by checking the box next to them.  After you select an attribute, it adds to the extracted XPath together with its value.

image:add-attribute.png[Add Attribute to XPath]

You can save your attribute selection for a web element to use it for future recorded events by checking the *Save selection*.
...
If you don't want to use the saved attribute or attributes for a given web element, click the image:default-icon.png[Default button] to revert back to the default extracted XPath for that particular web element.

=== Save Mouse Hover

You can record your mouse hover positioning while using the Web Recording Browser. This is particularly useful to capture navigation through drop down menus.

To capture a mouse over hover action durig web recording, press `F2` on an web element. This capturing is indicated with a blue flashing box around the selected web element.
