...
When you hover the cursor over a web element, the browser highlights it with a blue background. After interaction with an element (such as mouse click or key press on keyboard), its XPath and available attributes show on the Attribute Selection panel.
