...
To open the Process Recording Editor, select a process and click *Edit, Merge and Upload Recordings* (image:edit-process-button.png["", 2%, 2%]).
...
. Click *Load File* (image:load-file-button.png["", 2%, 2%]).
...
. Click *Save* (image:save-icon.png["", 2%, 2%]).
...
After merging and editing the BPMN and documentation, click the *Upload to RPA Manager* icon (image:upload-to-server-icon.png["", 2%, 2%]) to upload the new process BMPN. A message opens confirming that the upload is completed.

To close the Process Recording Editor and RPA Recorder, click *Close* (image:close-icon.png["", 2%, 2%]).
